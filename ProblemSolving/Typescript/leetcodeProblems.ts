import { ListNode, TreeNode, GraphNode, Track } from './Types';
...
function maxPathSum(root: TreeNode | null): number {
  ...
  function findMaxPath(node: TreeNode): number {
    ...
    const current_max_path = Math.max(node.val, node.val + leftSum, node.val + rightSum);
...
canFinish = (numCourses: number, prerequisites: number[][]): boolean => {
  ...
  for (let arr of prerequisites)
    if (!map.has(arr[0])) map.set(arr[0], { finite: null, nextCourses: [arr[1]] });
    else map.get(arr[0]).nextCourses.push(arr[1]);

  for (let crs of map.keys())
    if (!dfs(crs)) return false;
    else map.get(crs).finite = true;

  return true;

  function dfs(course: number): boolean {
    let currCrs = map.get(course);

    if (!currCrs || currCrs.finite) return true;
    if (visiting.has(course)) return false;

    visiting.add(course);

    for (let crs of currCrs.nextCourses) if (!dfs(crs)) return false;
...
var trap = (height: number[]): number => {
  const l = height.length;
  const leftMax = [height[0]];
  const rightMax = [height[l - 1]];
  let trappedWater = 0;
  let waterLevel = 0;
...
/**
 ** Graph
 * 3.8 Island Perimeter
 * https://leetcode.com/problems/island-perimeter/
 * TC|O(N) SC|O(N)
 */
function islandPerimeter(grid: number[][]): number {
  if (grid.length === 1 && grid[0].length === 0) return 4;
  ...
  function dfs(r: number, c: number): number {
    if (r >= grid.length || c >= grid[0].length || r < 0 || c < 0 || grid[r][c] === 0) return 1;
    if (grid[r][c] === 4) return 0;
